            ...
            <ul className="flex space-x-7 justify-center items-center cursor-pointer">
                {navItems.map((item, index) => (
                    <li key={index} className="font-serif flex items-center">
                        {item.name === 'Home' ? (
                            <Link activeClass="active" offset={-100} smooth spy to={item.to} duration={500} className="flex items-center">
                                <img src={item.photo} alt="Home Logo" className="h-10 w-10" />
                            </Link>
                        ) : (
                            <Link activeClass="active"  offset={-100} smooth spy to={item.to} duration={500}>
                                {item.name}
                            </Link>
                        )}
                    </li>
